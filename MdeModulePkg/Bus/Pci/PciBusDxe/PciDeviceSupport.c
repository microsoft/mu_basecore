...
{
  EFI_EVENT   ExitBootServicesEvent;    // MU_CHANGE - Disable BME on EBS
  EFI_STATUS  Status;                   // MU_CHANGE - Disable BME on EBS
  ...
  )
{
  ...
  if (RootBridge == NULL ) {
    // ASSERT (RootBridge != NULL);                    // MU_CHANGE - Disable BME on EBS
    ASSERT (PcdGetBool (PcdPciDisableBusEnumeration)); // MU_CHANGE - Disable BME on EBS
    return EFI_NOT_READY;
  }
