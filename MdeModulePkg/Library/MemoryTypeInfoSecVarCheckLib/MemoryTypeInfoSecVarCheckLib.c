...
// MU_SEC_TCBZ1086 [WHOLE FILE] - Mitigate potential system brick due to uefi MemoryTypeInformation var changes
  ...
  )
{
  ...
  if (DataSize != EFI_MEMORY_TYPE_INFORMATION_VARIABLE_SIZE) {
    DEBUG(( DEBUG_ERROR, "ERROR: %a() - DataSize = 0x%x Expected = 0x%x\n",
            __FUNCTION__, DataSize, EFI_MEMORY_TYPE_INFORMATION_VARIABLE_SIZE ));
  ...
  if (MemoryTypeInfo[Count - 1].Type != EfiMaxMemoryType) {
    DEBUG(( DEBUG_ERROR, "ERROR: %a() - Last(0x%x) entry's Type(0x%x) != EfiMaxMemoryType\n",
            __FUNCTION__, Count - 1, MemoryTypeInfo[Count - 1].Type ));
    return EFI_SECURITY_VIOLATION;
  }
  ...
  if (Count >= 2) {
    for (Index = 0; Index < Count - 1; Index++) {
      ...
      if (MemoryTypeInfo[Index].Type > EfiMaxMemoryType) {
        ...
        // The Type is invalid.
        //
        DEBUG(( DEBUG_ERROR, "ERROR: %a() - 0x%x entry's Type(0x%x) is invalid\n",
                __FUNCTION__, Index, MemoryTypeInfo[Index].Type ));
        return EFI_SECURITY_VIOLATION;
      }
      for (Index2 = Index + 1; Index2 < Count; Index2++) {
        if (MemoryTypeInfo[Index].Type == MemoryTypeInfo[Index2].Type) {
          ...
          //
          DEBUG(( DEBUG_ERROR, "ERROR: %a() - 0x%x entry and 0x%x entry have same Type(0x%x)\n",
                  __FUNCTION__, Index, Index2, MemoryTypeInfo[Index].Type ));
          return EFI_SECURITY_VIOLATION;
        }
      }

      if (EFI_ERROR( SafeUintnAdd( TotalPages, MemoryTypeInfo[Index].NumberOfPages, &TotalPages ) )) {
        DEBUG(( DEBUG_ERROR, "ERROR: %a() - Math error at entry 0x%x\n",
                __FUNCTION__, Index ));
        return EFI_SECURITY_VIOLATION;
      }
    }
  ...
  if (TotalPages > PcdGet32( PcdMaxMemoryTypeInfoPages )) {
    DEBUG(( DEBUG_ERROR, "ERROR: %a() - Total pages requested 0x%x exceeds max allowed(0x%x)\n",
            __FUNCTION__, TotalPages, PcdGet32( PcdMaxMemoryTypeInfoPages ) ));
    return EFI_SECURITY_VIOLATION;
  }
