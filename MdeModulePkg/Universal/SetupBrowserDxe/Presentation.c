  ...
  )
{
  ...
  while (!IsNull (&Form->StatementListHead, Link)) {
    ...
    if (Action == EFI_BROWSER_ACTION_CHANGING) {
      if (HiiValue->Type == EFI_IFR_TYPE_BUFFER) {
        BackUpBuffer = AllocateCopyPool(Statement->StorageWidth /* Fixing off by 2 error MSCHANGE */, Statement->BufferValue);
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (Action  == EFI_BROWSER_ACTION_CHANGING && Status == EFI_UNSUPPORTED) {
        if (HiiValue->Type == EFI_IFR_TYPE_BUFFER) {
          CopyMem (Statement->BufferValue, BackUpBuffer, Statement->StorageWidth /* Fixing off by 2 error MSCHANGE */);
        } else {
