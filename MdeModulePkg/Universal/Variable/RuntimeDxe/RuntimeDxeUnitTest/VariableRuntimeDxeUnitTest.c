  ...
  )
{
  ...
  if (!UnitTestAssertNotNull (VarA, __func__, DEBUG_LINE_NUMBER, __FILE__, "TestVarA")) {
  ...
  if (!UnitTestAssertEqual ((UINT64)(VarA->Attributes), (UINT64)(Attributes), __func__, DEBUG_LINE_NUMBER, __FILE__, "VarA->Attributes", "Attributes")) {
    TestResult = UNIT_TEST_ERROR_TEST_FAILED;
    goto Cleanup;
  }

  if (!UnitTestAssertEqual ((UINT64)(VarA->DataSize), (UINT64)(DataSize), __func__, DEBUG_LINE_NUMBER, __FILE__, "VarA->DataSize", "DataSize")) {
    TestResult = UNIT_TEST_ERROR_TEST_FAILED;
    goto Cleanup;
  }

  if (!UnitTestAssertMemEqual ((VOID *)VarA->Data, (VOID *)Data, (UINTN)VarA->DataSize, __func__, DEBUG_LINE_NUMBER, __FILE__, "VarA->Data", "Data")) {
    TestResult = UNIT_TEST_ERROR_TEST_FAILED;
    goto Cleanup;
  }
