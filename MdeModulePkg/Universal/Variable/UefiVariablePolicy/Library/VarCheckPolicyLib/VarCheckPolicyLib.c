...
/**
  ...
  NOTE: Unlike the Tianocore EdkiiVariableLockProtocol.RequestToLock, this will lock the variable
  immediately on request. It does not wait for EndOfDxe.
...
{
  EFI_STATUS              Status;
  VARIABLE_POLICY_ENTRY   *NewEntry = NULL;
  VOID                    *EntryName;
  UINTN                   NameSize;
  UINTN                   PolicySize;
  ...
  PolicySize = NameSize + sizeof(VARIABLE_POLICY_ENTRY);
  ...
  EntryName = (VOID*)(NewEntry + 1);

  DEBUG(( DEBUG_VERBOSE, "%a - RequestToLock -> RegisterVariablePolicy for %g:%s.\n",
          __FUNCTION__, VendorGuid, VariableName ));

  NewEntry->Version         = VARIABLE_POLICY_ENTRY_REVISION;
  NewEntry->Size            = (UINT16)PolicySize;
  NewEntry->OffsetToName    = sizeof(VARIABLE_POLICY_ENTRY);
  CopyGuid( &NewEntry->Namespace, VendorGuid );
  NewEntry->LockPolicyType  = VARIABLE_POLICY_TYPE_LOCK_NOW;
