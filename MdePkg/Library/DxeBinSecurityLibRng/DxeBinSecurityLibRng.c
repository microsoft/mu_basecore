...
#include <Protocol/StackCookieCheckFailureHook.h>
...
/**
  Locate the stack cookie failure protocol and execute the hook if it exists.
**/
VOID
EFIAPI
StackCookieFailure (
...
{
  EFI_STATUS                                Status;
  STACK_COOKIE_CHECK_FAILURE_HOOK_PROTOCOL  *StackCookieCheckFailureHookProtocol;

  Status = gBS->LocateProtocol (
                  &gStackCookieCheckFailureHookProtocolGuid,
                  NULL,
                  (VOID **)&StackCookieCheckFailureHookProtocol
                  );

  if (!EFI_ERROR (Status) &&
      (StackCookieCheckFailureHookProtocol->StackCookieCheckFailureHook != NULL))
  {
    StackCookieCheckFailureHookProtocol->StackCookieCheckFailureHook ();
  }
}
