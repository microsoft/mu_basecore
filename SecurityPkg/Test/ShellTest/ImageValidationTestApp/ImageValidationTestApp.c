...
// corrupt.
//
STATIC CONST UINT8  mCorruptSignaturePayload[SHA256_DIGEST_SIZE] = { 0 };
...
/**
  A BaseCryptLib one-shot hash routine (Sha256HashAll / Sha384HashAll / Sha512HashAll).
**/
typedef
BOOLEAN
...
/**
  Compute the hash of a certificate's TBSCertificate using the given hash routine.

  @param[in]  Cert      DER-encoded X.509 certificate.
  @param[in]  CertSize  Size, in bytes, of Cert.
  @param[in]  HashAll   BaseCryptLib ...HashAll routine that selects the algorithm.
  @param[out] Hash      Receives the digest; the caller's buffer must be large enough for the
                        chosen algorithm (up to SHA512_DIGEST_SIZE bytes).
  ...
  )
{
  ...
  if (!HashAll (TbsCert, TbsCertSize, Hash)) {
    return EFI_ABORTED;
  }
  ...
  )
{
  ...
  CONST struct {
    ...
    { DB_STATE_SIGNER1_LEAF_TBS_HASH_SHA256,          mLeafCert,             mLeafCertSize,             Sha256HashAll, &gEfiCertX509Sha256Guid, SHA256_DIGEST_SIZE },
    { DB_STATE_SIGNER1_LEAF_TBS_HASH_SHA384,          mLeafCert,             mLeafCertSize,             Sha384HashAll, &gEfiCertX509Sha384Guid, SHA384_DIGEST_SIZE },
    { DB_STATE_SIGNER1_LEAF_TBS_HASH_SHA512,          mLeafCert,             mLeafCertSize,             Sha512HashAll, &gEfiCertX509Sha512Guid, SHA512_DIGEST_SIZE },
    { DB_STATE_SIGNER1_INTERMEDIATE1_TBS_HASH_SHA256, mIntermediate1Cert,    mIntermediate1CertSize,    Sha256HashAll, &gEfiCertX509Sha256Guid, SHA256_DIGEST_SIZE },
    { DB_STATE_SIGNER1_INTERMEDIATE1_TBS_HASH_SHA384, mIntermediate1Cert,    mIntermediate1CertSize,    Sha384HashAll, &gEfiCertX509Sha384Guid, SHA384_DIGEST_SIZE },
    { DB_STATE_SIGNER1_INTERMEDIATE1_TBS_HASH_SHA512, mIntermediate1Cert,    mIntermediate1CertSize,    Sha512HashAll, &gEfiCertX509Sha512Guid, SHA512_DIGEST_SIZE },
    { DB_STATE_SIGNER1_INTERMEDIATE2_TBS_HASH_SHA256, mIntermediate2Cert,    mIntermediate2CertSize,    Sha256HashAll, &gEfiCertX509Sha256Guid, SHA256_DIGEST_SIZE },
    { DB_STATE_SIGNER1_INTERMEDIATE2_TBS_HASH_SHA384, mIntermediate2Cert,    mIntermediate2CertSize,    Sha384HashAll, &gEfiCertX509Sha384Guid, SHA384_DIGEST_SIZE },
    { DB_STATE_SIGNER1_INTERMEDIATE2_TBS_HASH_SHA512, mIntermediate2Cert,    mIntermediate2CertSize,    Sha512HashAll, &gEfiCertX509Sha512Guid, SHA512_DIGEST_SIZE },
    { DB_STATE_SIGNER1_ROOT_TBS_HASH_SHA256,          mRootCert,             mRootCertSize,             Sha256HashAll, &gEfiCertX509Sha256Guid, SHA256_DIGEST_SIZE },
    { DB_STATE_SIGNER1_ROOT_TBS_HASH_SHA384,          mRootCert,             mRootCertSize,             Sha384HashAll, &gEfiCertX509Sha384Guid, SHA384_DIGEST_SIZE },
    { DB_STATE_SIGNER1_ROOT_TBS_HASH_SHA512,          mRootCert,             mRootCertSize,             Sha512HashAll, &gEfiCertX509Sha512Guid, SHA512_DIGEST_SIZE },
    { DB_STATE_SIGNER2_TBS_HASH_SHA256,               mSigner2Cert,          mSigner2CertSize,          Sha256HashAll, &gEfiCertX509Sha256Guid, SHA256_DIGEST_SIZE },
    { DB_STATE_SIGNER2_TBS_HASH_SHA384,               mSigner2Cert,          mSigner2CertSize,          Sha384HashAll, &gEfiCertX509Sha384Guid, SHA384_DIGEST_SIZE },
    { DB_STATE_SIGNER2_TBS_HASH_SHA512,               mSigner2Cert,          mSigner2CertSize,          Sha512HashAll, &gEfiCertX509Sha512Guid, SHA512_DIGEST_SIZE },
    { DB_STATE_SIGNER2_UNRELATED_TBS_HASH_SHA256,     mSigner2UnrelatedCert, mSigner2UnrelatedCertSize, Sha256HashAll, &gEfiCertX509Sha256Guid, SHA256_DIGEST_SIZE },
    { DB_STATE_SIGNER2_UNRELATED_TBS_HASH_SHA384,     mSigner2UnrelatedCert, mSigner2UnrelatedCertSize, Sha384HashAll, &gEfiCertX509Sha384Guid, SHA384_DIGEST_SIZE },
    { DB_STATE_SIGNER2_UNRELATED_TBS_HASH_SHA512,     mSigner2UnrelatedCert, mSigner2UnrelatedCertSize, Sha512HashAll, &gEfiCertX509Sha512Guid, SHA512_DIGEST_SIZE }
  };
  SIG_LIST_SPEC       Specs[ARRAY_SIZE (DigestFlags) + ARRAY_SIZE (CertFlags) + ARRAY_SIZE (TbsFlags) + 2];
  UINT8               HashPool[ARRAY_SIZE (TbsFlags)][SHA512_DIGEST_SIZE];
  UINTN               HashCount;
  UINTN               Count;
  UINTN               Index;
  UINTN               TotalSize;
  ...
  *DatabaseSize = 0;
  ...
  if ((StateFlags & DB_STATE_CORRUPT_SIGNATURE_LIST_FIRST) != 0) {
    ...
    Specs[Count].DataSize  = SHA256_DIGEST_SIZE;
  ...
  //
  // Certificate TBSCertificate hashes, computed at runtime for the selected algorithm.
  //
  for (Index = 0; Index < ARRAY_SIZE (TbsFlags); Index++) {
    if ((StateFlags & TbsFlags[Index].Flag) != 0) {
      Status = ComputeTbsHash (
                 TbsFlags[Index].Cert,
                 TbsFlags[Index].CertSize,
                 TbsFlags[Index].HashAll,
                 HashPool[HashCount]
                 );
      if (EFI_ERROR (Status)) {
        return Status;
      ...
      Specs[Count].Data     = HashPool[HashCount];
  ...
  )
{
  ...
  for (ScenarioIndex = 0; ScenarioIndex < mScenarioCount; ScenarioIndex++) {
    ...
    if (EFI_ERROR (GenerateStatus)) {
      DEBUG ((
        DEBUG_ERROR,
        "Failed to generate scenario description for scenario %u. Status=%r\n",
