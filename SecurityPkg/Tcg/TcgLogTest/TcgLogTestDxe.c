...
/**
  ...
  (log base address changes) and the log is not truncated.
**/
STATIC
VOID
  ...
  )
{
  ...
  // Dump the event log and check for truncation.
  Status = TcgLogTestDumpEventLog (mTcg2Protocol, &Truncated);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: DumpEventLog failed - %r\n", __func__, Status));
    LogAppend ("FAIL: Pre-ReadyToBoot: DumpEventLog - %r\n", Status);
    return;
  }

  if (Truncated) {
    DEBUG ((DEBUG_ERROR, "%a: Log truncated after scaling\n", __func__));
    LogAppend ("FAIL: Pre-ReadyToBoot: Log truncated\n");
    return;
  }
  ...
  )
{
  ...
  TcgLogTestEnable (&mTcgLogTestProtocol, FALSE);
